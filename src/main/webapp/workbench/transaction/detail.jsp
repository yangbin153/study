...
<%@ page import="com.yangbin.crm.workbench.domain.Tran" %>
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if (data.success){
                            ...
                            html += '<font color="gray">交易</font> <font color="gray">-</font> <b>${tran.name}</b> <small style="color: gray;"> '+data.remark.createTime+' 由'+data.remark.createBy+'</small>';
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                $.each(data,function (i,n) {
                    ...
                    html += '<font color="gray">交易</font> <font color="gray">-</font> <b>${tran.name}</b> <small style="color: gray;"> '+(n.editFlag==0?n.createTime:n.editTime)+' 由'+(n.editFlag==0?n.createBy:n.editBy)+'</small>';
                    html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
                    html += '<a class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>';
                    html += 'nbsp;&nbsp;&nbsp;&nbsp;';
                    html += '<a class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>';
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
        ...
        <%
            ...
            //获取当前可能性
            String currentPossibilety = pMap.get(currentStage);
                //前7个为黑圈，后2个一个红x一个黑x
            if ("0".equals(currentPossibilety)){
                ...
                for (int i = 0; i < stageList.size(); i++) {
                    ...
                    //如果遍历出来的阶段的可能性为0，说明时后两个
                    if ("0".equals(listPossibility)){
                        //如果时当前阶段
                        if (listStage.equals(currentStage)){
                            //----------------红x
        %>
        <span class="glyphicon glyphicon-remove mystage" id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" data-toggle="popover" data-placement="bottom" data-content="<%=dicValue.getText()%>" style="color: #FF0000;"></span>
        -----------
        <%
                        }else {
                            //-----------------黑x
        %>
        <span class="glyphicon glyphicon-remove mystage" id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" data-toggle="popover" data-placement="bottom" data-content="<%=dicValue.getText()%>" style="color: #000000;"></span>
        -----------
        <%
                        }
                    //如果遍历出来的阶段的可能性不为0
                        ...
                        //----------黑圈
        %>
        <span class="glyphicon glyphicon-record mystage" id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" data-toggle="popover" data-placement="bottom" data-content="<%=dicValue.getText()%>" style="color: #000000;"></span>
        -----------
        <%
                    }
                }

                //前7个可能为绿圈，绿标，黑圈，后两个为黑x
            }else {
                //准备当前阶段下标
                int index = 0;
                for (int i = 0; i < stageList.size(); i++) {
        ...
        <%
                    ...
                    //如果遍历出来的阶段的可能性不为0，说明是前7个
                    }else {
                        //说明为现阶段之前
                        if (i<index){
                            //--------------绿圈
        %>
        <span class="glyphicon glyphicon-ok-circle mystage" id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" data-toggle="popover" data-placement="bottom" data-content="<%=dicValue.getText()%>" style="color: #90F790;"></span>
        -----------
        <%
                        }else if (i==index){
                            //---------------绿标
        %>
        <span class="glyphicon glyphicon-map-marker mystage" id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" data-toggle="popover" data-placement="bottom" data-content="<%=dicValue.getText()%>" style="color: #90F790;"></span>
        -----------
        <%
                        }else {
                            //----------------黑圈
